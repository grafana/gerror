...
// Always use [New] to create new instances of Base.
type Base struct {
...
// New initializes a [Base] that is used to construct [Error].
...
func New(reason Status, msgID string, opts ...BaseOpt) Base {
	b := Base{
		reason:    reason,
		messageID: msgID,
		logLevel:  reason.LogLevel(),
	}
...
// this [Base].
//
// Used as a functional option to [New].
...
// WithGuide sets the guide that will be attached to errors from this
// [Base].
func WithGuide(guide Guide) BaseOpt {
	return func(b Base) Base {
		b.guide = &guide
		return b
	}
...
// Status returns the reason used for the error.
...
//
//	// package-level
//	var errMonthlyQuota = New(gerror.StatusTooManyRequests, "service.monthlyQuotaReached")
//	// in function
...
//
//	// package-level
//	var errRateLimited = New(gerror.StatusTooManyRequests, "service.backoff").MustTemplate(
//		"quota reached for user {{ .Private.user }}, rate limited until {{ .Public.time }}",
//		gerror.WithPublic("Too many requests, try again after {{ .Public.time }}"),
//	)
//	// in function
...
type Error struct {
	...
	LogLevel LogLevel
	// Guide contains information aimed at supporting an end-user with
	// resolving common causes for the appearance of an issue.
	Guide *Guide
}
...
func (e Error) MarshalJSON() ([]byte, error) {
	return nil, fmt.Errorf("gerror.Error cannot be directly marshaled into JSON")
}
...
// Guide provides a structure for in-depth information aimed at
// supporting end-users with resolving an issue.
type Guide struct {
	// Text provides a written guide aimed at helping a user resolve an
	// error.
	Text string `json:"text"`
	// ImageURL provides a URL for an image resource that optionally
	// can be displayed together with the Text to give the user
	// additional guidance on how to resolve the error.
	ImageURL *url.URL `json:"imageUrl,omitempty"`
	// References is a list of hyperlinks to relevant resources.
	References map[string]url.URL `json:"references,omitempty"`
}
...
type PublicError struct {
	...
	// extra fields available to be written by the consumer
...
func (e Error) Public() PublicError {
	...
	if message == "" {
		if e.Reason == nil || e.Reason.Equal(StatusUnknown) {
	...
	return PublicError{
		StatusCode: e.Reason.HTTPStatus(),
